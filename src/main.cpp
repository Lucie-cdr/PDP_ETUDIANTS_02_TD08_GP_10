...
const char* wifi_ssid = "iPhone de Lucie";
const char* wifi_password = "coudiere";
...
const char* client_id = "TD08_GP10";
...
void setup() {
  ...
  if (mqtt_client.connect(client_id, mqtt_user, mqtt_pass)) {
    mqtt_client.publish("TD08_GP10/temp", String(temp_measure).c_str()); 
    mqtt_client.publish("TD08_GP10/relhum", String(relative_humidity_measure).c_str()); 
